...
app.post("/api/parse-properties", async (req, res) => {
  try {
    ...
    const filteredResponse = propertiesResponse.filter((item) => {
      const property = item.property;
      return (
        property.price?.value >= propertiesRequirements.price_starting &&
        property.price?.value <= propertiesRequirements.price_ending
      );
    });
